...
if ($scanText -notmatch '\[switch\]\$AllCandidates') {
...
if (Test-Path -LiteralPath $ps51) {
  ...
  try {
    $summary = & $ps51 -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'scripts\scan-drive.ps1') -DriveRoot $testDrive -OutputDirectory $audit -SkipUserContentScan -WriteReports
...
try {
  ...
  [pscustomobject]@{schemaVersion=3;targetRoot=[IO.Path]::GetPathRoot($protected.FullName);candidates=@([pscustomobject]@{groupId=1;sourceGroup=(Split-Path $protected.FullName -Parent);id='D0001';action='delete-low-risk';path=$protected.FullName;bytes=[long]$protected.Length;lastWriteUtc=$protected.LastWriteTimeUtc.ToString('o')})} | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $manifest -Encoding UTF8
  & $ps51 -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'scripts\apply-approved.ps1') -Manifest $manifest -GroupIds 1 -ResultPath $result | Out-Null
