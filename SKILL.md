...
2. Run `scripts/scan-drive.ps1 -DriveRoot <drive> -AllCandidates` without elevation unless the user explicitly asks for an age-filtered scan. Discovery must be read-only. Keep the JSON manifest as an internal execution input and do not generate Markdown or CSV unless the user explicitly requests report files. The scan records size and timestamp only; SHA-256 is deferred until the user approves an actual move.
