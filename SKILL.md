...
2. Run `scripts/scan-drive.ps1 -DriveRoot <drive>` without elevation. Its defaults apply no age, file-size, or candidate-count filter; `-AllCandidates` remains an explicit alias for the same behavior. Discovery must be read-only. Keep the JSON manifest as an internal execution input and do not generate Markdown or CSV unless the user explicitly requests report files. The scan records size and timestamp only; SHA-256 is deferred until the user approves an actual move.
